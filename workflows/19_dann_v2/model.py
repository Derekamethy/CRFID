"""Canonical C1 adapter and magnitude-matched DANN v2 domain controls."""
...
class _GradientScale(torch.autograd.Function):
    @staticmethod
    def forward(ctx: object, inputs: torch.Tensor, coefficient: float) -> torch.Tensor:
...
def scale_encoder_gradient(inputs: torch.Tensor, coefficient: float) -> torch.Tensor:
    """Forward identity with an explicit encoder-side backward coefficient."""

    return _GradientScale.apply(inputs, float(coefficient))


def gradient_reverse(inputs: torch.Tensor, strength: float) -> torch.Tensor:
    return scale_encoder_gradient(inputs, -float(strength))


def gradient_forward_matched(inputs: torch.Tensor, strength: float) -> torch.Tensor:
    return scale_encoder_gradient(inputs, float(strength))
...
class C1PositionDANN(nn.Module):
    ...
    def forward_domain_treatment(
        self, inputs: torch.Tensor, *, encoder_domain_coefficient: float
        ...
        domain_logits = self.domain_head(
            scale_encoder_gradient(embeddings, encoder_domain_coefficient)
        )
    ...
    def forward_dann(
        ...
        """Compatibility spelling for the negative DANN arm."""

        return self.forward_domain_treatment(
            inputs, encoder_domain_coefficient=-float(grl_strength)
        )
